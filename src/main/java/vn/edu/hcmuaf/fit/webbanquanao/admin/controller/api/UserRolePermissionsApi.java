...
public class UserRolePermissionsApi extends BaseApiServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ApiContext ctx = initContext(req, resp, "UserRolePermission");
    ...
    protected void doPut(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        try {
            ...
            logService.logCustom(ctx.username, "INFO",
                    "Yêu cầu cập nhật quyền cho user=" + updated.getUserName() + ", NewRoles=" + updated.getRoles(),
                    ctx.ip, ctx.roles);

            boolean success = userService.updateUserRolesWithPermissionCheck(
                    currentUser, updated.getUserName(), updated.getRoles());
            if (success) {
                logService.logUpdateEntity(ctx.username, "UserRolePermission", updated.getUserName(), ctx.ip, ctx.roles);
                sendSuccess(resp, HttpServletResponse.SC_OK, "Cập nhật thành công");
            } else {
                logService.logCustom(ctx.username, "WARN",
                        "Không có quyền thay đổi roles cho user=" + updated.getUserName(),
                        ctx.ip, ctx.roles);
                sendError(resp, HttpServletResponse.SC_FORBIDDEN, "Bạn không có quyền thay đổi vai trò");
            }
